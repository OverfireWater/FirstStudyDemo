...
if ($_GET['id']){
    $id=$_GET['id'];
    $con=$db->queryData("select * from shopClassify where id=".$id);
    $classify_arr=$db->queryData("select * from shopClassify_const ");
}
?>
<body>
<script>
    $(function (){
        $("#selectUser option[value='<?=$con[0]['parent']?>']").prop("selected","selected");
    });
</script>
<div class="container">
    <div class="wrap">
                    ...
                    <form class="layui-form">
                        <div class="shop_add-width">
                            <div class="shop-add-border-margin">
                                ...
                                <div class="shop-name">
                                    <span style="display: inline-block;width: 72px">修改类型：</span>
                                    <label>
                                        <input type="text" id="classify_name" name="name" value="<?=$con[0]['classifyName']?>" class="input-shop" placeholder="请输入类型">
                                ...
                                <div class="shop-name" style="display: inline-block;">
                                    <span style="margin-bottom: 100px;display: block;float:left;width: 72px">类型备注：</span>
                                    <label>
    ...
    $(function (){
        ...
        $("#shop_but").click(function (){
            ...
            if (classify_name!="" && classify_name!=null ){
                $.ajax({
                    type:"post",
                    url:"../ClassifyServices/UdClassifyServices.php",
                    data:{"classify_name":classify_name,"remark":$("#remark").val(),"Parent_classify":$('#selectUser').val(),'id':<?=$id?>},
                    dataType:"text",
